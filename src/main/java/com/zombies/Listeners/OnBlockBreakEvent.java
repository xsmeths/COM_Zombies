...
public class OnBlockBreakEvent implements Listener {
	...
	public void onBlockBreakEvent(BlockBreakEvent interact) {
				...
				if (interact.getBlock().getLocation().equals(point.getLocation())) {
					...
					if (game.spawnManager.getPoints().size() == 0) {
						CommandUtil.sendMessageToPlayer(player,
								ChatColor.RED + "No spawns left! Force canceling this operation!");
						String[] args = new String[2];
						args[0] = "cancel";
						args[1] = "removespawn";
						plugin.command.onRemoteCommand(player, args);
		...
		if (manager.isPlayerInGame(player)) {
			if (interact.getBlock().getType().toString().toLowerCase().contains("sign")) {
		...
		if (interact.getBlock().getType().toString().toLowerCase().contains("sign")) {
			...
			if (ChatColor.stripColor(lineOne).equalsIgnoreCase("[Zombies]")
					&& ChatColor.stripColor(lineTwo).equalsIgnoreCase("MysteryBox")) {
