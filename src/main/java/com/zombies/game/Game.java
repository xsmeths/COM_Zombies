...
import com.zombies.game.managers.*;
...
public class Game {
	...
	/**
	 * Assuring that the game has every point, point one for the arena, and point
	 * two.
	 */
	...
	/**
	 * contains all of the perks in the game as well as the perks that are currently
	 * dropped in the map
	 */
	...
	/**
	 * Information containing gamemode, and fly mode the player was in before they
	 * joined the game.
	 */
	...
	/**
	 * Max players is used to check for player count and if not to remove a player
	 * if the game is full.
	 */
	...
	public void nextWave() {
		...
		if (!(spawnManager.getZombiesAlive() == 0)
				|| !(spawnManager.getZombiesToSpawn() <= spawnManager.getZombiesSpawned()))
			return;
		if (changingRound)
			return;
		changingRound = true;
	...
	public void addPlayer(Player player) {
		if (mode == ArenaStatus.WAITING || mode == ArenaStatus.STARTING) {
			...
			for (Player pl : players) {
				CommandUtil.sendMessageToPlayer(pl,
						player.getName() + " has joined with " + players.size() + "/" + maxPlayers + "!");
			}
			if (players.size() >= plugin.configManager.getConfig("ArenaConfig").getInt(arenaName + ".minPlayers")) {
				if (starter == null) {
					...
					for (Player pl : players) {
						CommandUtil.sendMessageToPlayer(pl,
								ChatColor.RED + "" + ChatColor.BOLD + "Game starting soon!");
					}
					mode = ArenaStatus.STARTING;
				} else {
					if (starter.started) {
			...
			CommandUtil.sendMessageToPlayer(player,
					"Something could have went wrong here, COM Zombies has picked this up and will continue without error.");
		}
		signManager.updateGame();
	}
	...
	public void removePlayer(Player player) {
		...
		for (Player pl : players) {
			if (!isDisabled)
				CommandUtil.sendMessageToPlayer(pl, player.getName() + " has left the game! Only " + players.size()
						+ "/" + this.maxPlayers + " player(s) left!");
		}
		if (players.size() == 0) {
			if (!isDisabled) {
				mode = ArenaStatus.WAITING;
	...
	public void assignPlayerInventory(Player player) {
		...
		ItemStack ib = new ItemStack(Material.GLASS_PANE, 1);
	...
	public void updateBarrierDamage(int damage, Block block) {
		for (Player player : this.players) {
			PacketPlayOutBlockBreakAnimation packet = new PacketPlayOutBlockBreakAnimation(0,
					new BlockPosition(block.getX(), block.getY(), block.getZ()), damage);
			DimensionManager dimension = DimensionManager.OVERWORLD;
			((CraftServer) player.getServer()).getHandle().sendPacketNearby(null, block.getX(), block.getY(),
					block.getZ(), 120, dimension, packet);
		}
	}
}
