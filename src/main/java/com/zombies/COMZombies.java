...
public class COMZombies extends JavaPlugin {
	...
	/**
	 * Players who are contained in this hash map are remove spawns, the value that
	 * player contains is the game that they are removing spawns from.
	 */
	public HashMap<Player, Game> isRemovingSpawns = new HashMap<Player, Game>();
	/**
	 * Players who are in this hash map are creating a door for a game, the value
	 * that player contains is the door that they are creating. The door contains
	 * the game and it's information.
	 */
	 ...
	 * arena, the value that corresponds to the player is the game that the player
	 * is removing doors from.
	 */
	...
	/**
	 * Players who are contained in this hash map are in sign edit for a given sign,
	 * the value that corresponds to the player is the sign that the player is
	 * editing.
	 */
	...
	public static String prefix = ChatColor.RED + "< " + ChatColor.GOLD + ChatColor.ITALIC + "CoM: Zombies"
	...
	/**
	 * Configuration setup, gathers all information from the configuration files and
	...
	public void onEnable() {
		...
		Bukkit.broadcastMessage(
				ChatColor.RED + "[Zombies] " + ChatColor.GREEN + "" + ChatColor.BOLD + "This server is running "
						+ ChatColor.GOLD + "" + ChatColor.BOLD + getName() + ChatColor.RED + "" + ChatColor.BOLD + "!");
		Bukkit.broadcastMessage(
				ChatColor.RED + "[Zombies] " + ChatColor.GREEN + "" + ChatColor.BOLD + "Testing plugin...");
		try {
			testPlugin();
		} catch (Exception e) {
			Bukkit.broadcastMessage(
					ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED + "Zombies has run into an error!");
			Bukkit.broadcastMessage(ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED + e.toString());

			say = false;
		}
		if (say) {
			Bukkit.broadcastMessage(ChatColor.RED + "[Zombies] " + ChatColor.GREEN + "" + ChatColor.BOLD
					+ "Zombies is working just fine!");
			saveConfig();
		}
		...
		/*if (getConfig().getBoolean("config.settings.checkForUpdates")) {
			try {
				...
				Bukkit.broadcastMessage(ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED
						+ "COM:Z has run into an issue connection to the bukkit page");
				return;
			}
			try {
				...
				if (array.size() > 0) {
					...
					if (!getDescription().getVersion().equalsIgnoreCase(versionName)) {
						Bukkit.broadcastMessage(ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED + "" + ChatColor.BOLD
								+ "There is an update availible for COM:Z that differs from yours!");
						System.out.println(ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED + "" + ChatColor.BOLD
								+ "You currently have version: " + getDescription().getVersion()
								+ " and the current version fro COM:Z is verion: " + versionName);
						System.out.println(ChatColor.RED + "[Zombies] " + ChatColor.DARK_RED + "" + ChatColor.BOLD
								+ "Get the latest version of COM:Z here: " + versionLink);
					}
	...
	/**
	 * Tests the plugin for any errors, if an exception is caught, the server will
