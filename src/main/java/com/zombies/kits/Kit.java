		...
		if (config.getString(name + ".Guns") != null) {
				...
				if (guns[0] != null) {
					...
					if (gun == null)
						Bukkit.broadcastMessage(
								ChatColor.RED + "[Zombies] Kit Gun: " + guns[0] + "  is an invalid gun name!");
					gunOne = gun;
				}
			...
			try {
				if (guns[1] != null) {
					...
					if (gun == null)
						Bukkit.broadcastMessage(
								ChatColor.RED + "[Zombies] Kit Gun: " + guns[1] + "  is an invalid perk name!");
					gunTwo = gun;
				}
			} catch (ArrayIndexOutOfBoundsException e) {
				e.addSuppressed(e.getCause());
			...
			try {
				if (guns[2] != null) {
					...
					if (gun == null)
						Bukkit.broadcastMessage(
								ChatColor.RED + "[Zombies] Kit Gun: " + guns[2] + "  is an invalid perk name!");
					gunThree = gun;
				}
			} catch (ArrayIndexOutOfBoundsException e) {
				e.addSuppressed(e.getCause());
		...
		if (config.getString(name + ".Perks") != null) {
			...
			try {
				if (perks[0] != null) {
					...
					if (perk == null)
						Bukkit.broadcastMessage(
								ChatColor.RED + "[Zombies] Perk: " + perkName + "  is an invalid perk name!");
					perkOne = perk;
				}
			} catch (ArrayIndexOutOfBoundsException e) {
				e.addSuppressed(e.getCause());
